...
function renderSettings() {
  ...
  Object.entries(state.examSets).forEach(([id, es]) => {
    ...
    setHtml += `<div class="exam-set-card"><span>${es.name} <span style="font-size:11px;color:var(--text2)">(${es.type})</span></span><label class="exam-set-toggle-label"><input type="checkbox" class="exam-set-toggle" data-id="${id}" ${isEnabled ? 'checked' : ''}><span class="toggle-slider"></span></label><button class="btn btn-small exam-set-switch" data-id="${id}" ${id === setId ? 'disabled' : ''}>Select</button>${es.type === 'builtin' ? '' : '<button class="btn btn-small btn-danger exam-set-delete" data-id="' + id + '">&times;</button>'}</div>`;
  });
  setHtml += '</div>';
