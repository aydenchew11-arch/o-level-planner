...
(async () => {
  ...
  // First visit detection — force skip if user already has data
  try {
    const rawData = localStorage.getItem(STORAGE_KEY);
    if (rawData) {
      const parsed = JSON.parse(rawData);
      const hasRealData = parsed && parsed.examSets && Object.keys(parsed.examSets).length > 0;
      if (hasRealData) {
        state.firstVisit = false;
        // Restore any exam sets that may have been lost (e.g. after migration corruption)
        Object.assign(state.examSets, parsed.examSets);
        if (parsed.activeSetId && !state.activeSetId) state.activeSetId = parsed.activeSetId;
      }
    }
  } catch {}
  if (state.firstVisit) {
