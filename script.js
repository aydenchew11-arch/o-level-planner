...
(async () => {
  ...
  // Robust first-visit detection: skip wizard if user already has selected subjects
  const hasSelectedSubjects = Object.values(state.examSets).some(s => (s.selectedIds || []).length > 0);
  if (hasSelectedSubjects) state.firstVisit = false;
  if (state.firstVisit) {
