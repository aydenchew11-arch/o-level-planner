...
function computeSubjectResult(setId, subject) {
  ...
  return { papers, weights, scores, target, minPct, knownPts, knownW, remW, remCount, currentPct, projectedPct, needRel, achievable };
}
function resultsStatusHtml(r) {
  if (r.knownW === 0) return '<div class="results-status muted">Enter scores for some papers to see what you need for your target grade.</div>';
  if (r.remW <= 0) {
    const ok = r.currentPct >= r.minPct;
    return `<div class="results-status ${ok ? 'ok' : 'bad'}">All papers entered &mdash; weighted score <strong>${r.currentPct.toFixed(1)}%</strong>. ${ok ? 'On track for <strong>' + r.target + '</strong>.' : 'Below <strong>' + r.target + '</strong> (' + r.minPct + '% needed).'}</div>`;
  }
  if (!r.achievable) return `<div class="results-status bad">Target <strong>${r.target}</strong> (${r.minPct}%) is not achievable &mdash; you already lost too many marks.</div>`;
  if (r.needRel <= 0) return `<div class="results-status ok">Already on track for <strong>${r.target}</strong> (${r.minPct}%). Keep it up!</div>`;
  return `<div class="results-status need">To hit <strong>${r.target}</strong> (${r.minPct}%), you need about <strong>${Math.min(100, r.needRel).toFixed(1)}%</strong> on the remaining paper${r.remCount > 1 ? 's' : ''}.</div>`;
}
function renderResultsSubject(setId, subject) {
  ...
  const targetOptions = scale.grades.map(([g, min]) => `<option value="${g}" ${r.target === g ? 'selected' : ''}>${g} (${min}+%)</option>`).join('');
  const rows = r.papers.map(p => {
    const w = r.weights[p.id];
    const s = r.scores[p.id] !== undefined && r.scores[p.id] !== '' ? r.scores[p.id] : '';
    return `<div class="results-row"><span class="results-paper">${p.paper}${p.paperName && p.paperName !== p.paper ? ' <span class="results-paper-sub">' + p.paperName + '</span>' : ''}</span><input type="number" min="0" max="100" step="0.5" class="results-weight-input" data-set="${setId}" data-paper="${p.id}" value="${Number(w).toFixed(w % 1 ? 1 : 0)}" title="Paper weighting (%)" aria-label="Weighting for ${p.paper}"><input type="number" min="0" max="100" step="0.5" class="results-score-input" data-set="${setId}" data-paper="${p.id}" placeholder="Score %" value="${s}" aria-label="Score for ${p.paper}"></div>`;
  }).join('');
  const sid = setId + '-' + subject.replace(/[^a-z0-9]/gi, '_');
  return `<div class="results-card" style="border-left-color:${color}"><div class="results-card-head"><div class="results-subject" style="color:${color}">${subject}</div><label class="results-target"><span>Target</span><select class="results-target-select" data-set="${setId}" data-subject="${subject}">${targetOptions}</select></label></div><div class="results-rows">${rows}</div><div id="results-status-${sid}" class="results-status-wrap">${resultsStatusHtml(r)}</div><div class="results-note">Weights shown come from syllabus defaults where known &mdash; edit to match yours.</div></div>`;
}
function renderResults() {
  ...
  sets.forEach(([id, set]) => {
    ...
    html += `<div class="results-set"><div class="results-set-title"><h3>${set.name}</h3><span class="results-scale-badge">${scale.name} grading</span></div>`;
...
function updateResultsStatus(setId, subject) {
  const r = computeSubjectResult(setId, subject);
  const el = document.getElementById('results-status-' + setId + '-' + subject.replace(/[^a-z0-9]/gi, '_'));
  if (el) el.innerHTML = resultsStatusHtml(r);
}
...
(async () => {
  ...
  // View tabs
  document.querySelectorAll('.view-tab').forEach(tab => { tab.addEventListener('click', () => { document.querySelectorAll('.view-tab').forEach(t => t.classList.remove('active')); tab.classList.add('active'); document.querySelectorAll('.view-content').forEach(v => v.classList.remove('active')); const view = tab.dataset.view; if (view === 'calendar') document.getElementById('calendar-view').classList.add('active'); else if (view === 'notes') { document.getElementById('notes-view').classList.add('active'); renderNotes(); } else if (view === 'plan') document.getElementById('plan-view').classList.add('active'); else if (view === 'results') { document.getElementById('results-view').classList.add('active'); renderResults(); } else document.getElementById('list-view').classList.add('active'); refreshAll(); }); });
...
// Results view inputs
  ...
  resultsView.addEventListener('input', (e) => {
    const t = e.target;
    if (!t.classList.contains('results-score-input') && !t.classList.contains('results-weight-input')) return;
    ...
    const val = t.value;
    if (val === '' || val === null) delete set[key][t.dataset.paper];
    else if (!isNaN(Number(val))) set[key][t.dataset.paper] = Math.min(100, Math.max(0, Number(val)));
    saveState();
    const subject = getSetPapers(t.dataset.set).find(p => p.id === t.dataset.paper);
    if (subject) updateResultsStatus(t.dataset.set, subject.subject);
  });
  resultsView.addEventListener('change', (e) => {
    const t = e.target;
    if (!t.classList.contains('results-target-select')) return;
    const set = state.examSets[t.dataset.set]; if (!set) return;
    set.resultTargets[t.dataset.subject] = t.value;
    saveState();
    updateResultsStatus(t.dataset.set, t.dataset.subject);
  });
