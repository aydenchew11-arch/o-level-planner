...
// ===== BUILT-IN EXAM DATA (SEAB 2026 GCE O-Level) =====
const EXAM_DATA_BUILTIN = {
  'o-level': { name: 'GCE O-Level 2026', CATEGORIES, subjects: DEFAULT_SUBJECT_COLORS, papers: [
...
const GOAL_TEMPLATES = [
  'Complete TYS (past 5 years)', 'Review all lecture notes', 'Practice Section A questions',
  'Practice Section B questions', 'Create summary mind maps', 'Do timed practice papers',
  'Focus on weak topics', 'Review past mistakes', 'Memorise key formulas', 'Study with flashcards'
];
...
const DEFAULTS = {
  user: null, email: '', examSets: {}, activeSetId: '',
  darkMode: false, notifications: false, notificationsEnabled: false, firstVisit: true,
  syncTimestamp: null, googleFormUrl: GOOGLE_FORM_URL
};
let state = {};
let syncTimer = null;
...
function getPapers() {
  const set = state.examSets[state.activeSetId];
  if (!set) return [];
  if (set.type === 'builtin') {
    const builtin = EXAM_DATA_BUILTIN[set.builtinKey];
...
function getCategories() {
  ...
  if (set.type === 'builtin') {
    const builtin = EXAM_DATA_BUILTIN[set.builtinKey];
    return builtin ? builtin.CATEGORIES || CATEGORIES : {};
  }
  return {};
}
...
function getSubjectColors() {
  const set = state.examSets[state.activeSetId];
  if (!set) return {};
  if (set.type === 'builtin') return { ...DEFAULT_SUBJECT_COLORS, ...set.subjectColors };
  return set.subjectColors || {};
}

function getDefaultColors(subject) {
  return DEFAULT_SUBJECT_COLORS[subject] || '#6b7280';
}

function getSelectedExams() {
  return getPapers().filter(e => {
    const set = state.examSets[state.activeSetId];
    return set && set.selectedIds && set.selectedIds.includes(e.id);
  });
}

function getSetProgress() { const s = state.examSets[state.activeSetId]; return s ? s.progress || {} : {}; }
function getSetNotes() { const s = state.examSets[state.activeSetId]; return s ? s.notes || {} : {}; }
function getSetGoals() { const s = state.examSets[state.activeSetId]; return s ? s.goals || {} : {}; }
...
function saveState(sync) {
  ...
  if (sync !== false && currentUser && supabase) {
...
async function syncToSupabase() {
  ...
  try {
    state.syncTimestamp = new Date().toISOString();
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
    const payload = { user_id: currentUser.id, data: state, updated_at: state.syncTimestamp };
...
async function loadFromSupabase() {
  if (!currentUser || !supabase) return;
  try {
    const { data, error } = await supabase.from('user_data').select('data').eq('user_id', currentUser.id).single();
    if (error && error.code !== 'PGRST116') { console.warn('Supabase fetch error:', error.message); return; }
    if (data && data.data) {
      const remote = data.data;
      if (!state.syncTimestamp || (remote.syncTimestamp && remote.syncTimestamp > state.syncTimestamp)) {
...
// ===== AUTH =====
function showAuthView() { document.getElementById('login-overlay').classList.remove('hidden'); document.getElementById('app').classList.add('hidden'); }
function showAppView() { document.getElementById('login-overlay').classList.add('hidden'); document.getElementById('app').classList.remove('hidden'); }

async function initAuth() {
  if (!supabase) { showAppView(); return; }
  try {
...
function skipLogin() {
  ...
  if (!state.activeSetId) {
    state.examSets['builtin-o-level'] = { name: 'GCE O-Level 2026', type: 'builtin', builtinKey: 'o-level', selectedIds: [], progress: {}, notes: {}, goals: {}, subjectColors: {} };
    state.activeSetId = 'builtin-o-level';
  }
  saveState(false);
  showAppView();
  refreshAll();
...
// ===== EXAM SET MANAGEMENT =====
function addBuiltinExamSets() {
  let changed = false;
  if (!state.examSets['builtin-o-level']) {
    state.examSets['builtin-o-level'] = { name: 'GCE O-Level 2026', type: 'builtin', builtinKey: 'o-level', selectedIds: [], progress: {}, notes: {}, goals: {}, subjectColors: {} };
    changed = true;
  }
  if (!state.activeSetId) { state.activeSetId = 'builtin-o-level'; changed = true; }
...
function createCustomExamSet(name, papers) {
  ...
  const colors = {};
  uniqueSubjects.forEach(s => { colors[s] = getDefaultColors(s); });
  state.examSets[id] = {
    name, type: 'custom', papers: papers.map((p, i) => ({
      id: id + '-p' + i,
      subject: p.subject,
      code: p.code || '',
      paper: p.paper || '',
      paperName: p.paperName || p.paper || '',
      date: p.date,
      startTime: p.startTime || null,
      endTime: null, mode: 'Written', duration: '', notes: ''
    })),
    selectedIds: [],
    progress: {}, notes: {}, goals: {}, subjectColors: colors
  };
...
function deleteExamSet(id) {
  if (state.examSets[id].type === 'builtin') { showToast('Cannot delete built-in exam sets.', 3000); return; }
  if (!confirm('Delete exam set "' + state.examSets[id].name + '"? This cannot be undone.')) return;
...
// ===== GOALS CRUD =====
function getGoals(subject) { const g = getSetGoals(); return g[subject] || []; }
function addGoal(subject, text) {
  if (!text.trim()) return;
  const set = state.examSets[state.activeSetId];
  if (!set) return;
  if (!set.goals[subject]) set.goals[subject] = [];
  set.goals[subject].push({ id: Date.now().toString(36) + Math.random().toString(36).slice(2,6), text: text.trim(), completed: false });
  saveState();
}
function toggleGoal(subject, goalId) { const g = getSetGoals(); if (!g[subject]) return; const goal = g[subject].find(x => x.id === goalId); if (goal) { goal.completed = !goal.completed; saveState(); } }
function deleteGoal(subject, goalId) { const g = getSetGoals(); if (!g[subject]) return; g[subject] = g[subject].filter(x => x.id !== goalId); if (!g[subject].length) delete g[subject]; saveState(); }

// ===== RENDER HELPERS =====
function getSubjectColor(subject) { const c = getSubjectColors(); return c[subject] || getDefaultColors(subject); }
...
function getTrafficHTML(examId) {
  const levels = ['red', 'yellow', 'green'], labels = ['Not Started', 'Revising', 'Ready'];
  const p = getSetProgress();
  const current = p[examId] || 'red';
  return `<div class="traffic-lights" data-exam="${examId}">${levels.map((l, i) => `<span class="traffic-light traffic-${l}${l === current ? ' active' : ''}" data-level="${l}" title="${labels[i]}"></span>`).join('')}<span class="traffic-label">${labels[levels.indexOf(current)]}</span></div>`;
}
function getTrafficSelectHTML(examId) {
  const levels = ['red', 'yellow', 'green'], labels = ['Not Started', 'Revising', 'Ready'];
  const p = getSetProgress();
  const current = p[examId] || 'red';
  return levels.map((l, i) => `<span class="traffic-light traffic-${l}${l === current ? ' active' : ''}" data-level="${l}" title="${labels[i]}"></span>`).join('');
}
...
function renderHero() {
  ...
  const days = daysUntilExam(next), status = getExamStatus(next), prog = getPeriodProgress();
...
function renderExamCard(exam) {
  ...
  const note = getSetNotes()[exam.id] || '';
...
function renderGoals() {
  ...
  const isCollapsed = state.examSets[state.activeSetId]?.goalsCollapsed;
  ...
  subjSet.forEach(subj => {
    const color = getSubjectColor(subj), goals = getGoals(subj);
    ...
    if (goals.length) goals.forEach(g => { html += `<div class="goal-item"><input type="checkbox" class="goal-check" data-subject="${subj}" data-id="${g.id}" ${g.completed ? 'checked' : ''}><span class="goal-text${g.completed ? ' done' : ''}">${g.text}</span><button class="goal-delete" data-subject="${subj}" data-id="${g.id}" title="Delete goal">&times;</button></div>`; });
    html += `<div class="goal-add-row"><input type="text" class="goal-new-input" data-subject="${subj}" placeholder="Add a study goal..." maxlength="200"><select class="goal-template-select" data-subject="${subj}"><option value="">Template...</option>${GOAL_TEMPLATES.map(t => `<option value="${t}">${t}</option>`).join('')}</select><button class="btn btn-primary goal-add-btn" data-subject="${subj}">+ Add</button></div></div>`;
  });
  container.innerHTML = html;
  container.querySelectorAll('.goal-check').forEach(cb => { cb.addEventListener('change', () => { toggleGoal(cb.dataset.subject, cb.dataset.id); renderGoals(); }); });
  container.querySelectorAll('.goal-delete').forEach(btn => { btn.addEventListener('click', () => { deleteGoal(btn.dataset.subject, btn.dataset.id); renderGoals(); }); });
  ...
  container.querySelectorAll('.goal-add-btn').forEach(btn => { btn.addEventListener('click', () => { const row = btn.closest('.goal-add-row'); const inp = row.querySelector('.goal-new-input'); addGoal(btn.dataset.subject, inp.value); inp.value = ''; renderGoals(); }); });
  container.querySelectorAll('.goal-new-input').forEach(inp => { inp.addEventListener('keydown', (e) => { if (e.key === 'Enter') { addGoal(inp.dataset.subject, inp.value); inp.value = ''; renderGoals(); } }); });
}

function renderList(ids) {
  const exams = ids.map(id => getPapers().find(e => e.id === id)).filter(Boolean);
...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (examsToday.length) { html += `<div class="cal-dots">`; examsToday.forEach(ex => { html += `<span class="cal-dot" style="background:${getSubjectColor(ex.subject)}" title="${ex.subject} ${ex.paper}"></span>`; }); html += `</div>`; }
  ...
  document.querySelectorAll('.cal-day:not(.other-month)').forEach(el => {
    el.addEventListener('click', () => {
      ...
      renderCalendar();
      const examsToday = getSelectedExams().filter(e => e.date === el.dataset.date);
      const panel = document.getElementById('cal-exam-detail');
      if (!examsToday.length) { panel.innerHTML = `<p class="cal-no-selection">No exams on ${formatDate(el.dataset.date)}.</p>`; return; }
      panel.innerHTML = `<div style="font-size:14px;font-weight:600;margin-bottom:10px">${formatDate(el.dataset.date)}</div>` + examsToday.map(ex => `<div class="cal-exam-item" data-id="${ex.id}"><span class="subj-dot" style="background:${getSubjectColor(ex.subject)}"></span><span><strong>${ex.subject}</strong> ${ex.paper}${ex.startTime ? ' &middot; ' + formatTime(ex.startTime) : ''}</span></div>`).join('');
      panel.querySelectorAll('.cal-exam-item').forEach(item => { item.addEventListener('click', () => showExamDetail(item.dataset.id)); });
    });
  });
}
...
function renderSettings() {
  ...
  Object.entries(state.examSets).forEach(([id, es]) => {
    setHtml += `<div class="exam-set-card"><span>${es.name}${id === state.activeSetId ? ' <strong>(active)</strong>' : ''} <span style="font-size:11px;color:var(--text2)">(${es.type})</span></span><button class="btn btn-small exam-set-switch" data-id="${id}">Switch</button><button class="btn btn-small btn-danger exam-set-delete" data-id="${id}" ${es.type === 'builtin' ? 'disabled style="opacity:0.3"' : ''}>&times;</button></div>`;
  });
  setHtml += '</div>';
  setContainer.innerHTML = setHtml;
  setContainer.querySelectorAll('.exam-set-switch').forEach(btn => { btn.addEventListener('click', () => { switchExamSet(btn.dataset.id); renderSettings(); }); });
  ...
  if (papers.length && set.type === 'builtin') {
    ...
    Object.entries(cats).forEach(([cat, subjects]) => {
      const entries = subjects.map(s => {
        ...
        const allSelected = pts.every(p => set.selectedIds.includes(p.id));
        return `<div class="setup-group" data-subject="${s}"><div class="setup-group-header"><input type="checkbox" class="subject-check" ${allSelected ? 'checked' : ''}><label>${s}</label></div><div class="setup-papers">${pts.map(p => `<div class="setup-paper"><input type="checkbox" class="paper-check" data-id="${p.id}" ${set.selectedIds.includes(p.id) ? 'checked' : ''}><label>${p.paper}${p.paperName ? ' &mdash; ' + p.paperName : ''}</label></div>`).join('')}</div></div>`;
      }).filter(Boolean);
    ...
    ['Languages', 'Mathematics', 'Sciences', 'Humanities', 'Arts & Electives'].forEach(cat => { if (groupsByCat[cat]) subjContainer.innerHTML += `<div class="setup-category"><div class="setup-category-title">${cat}</div>${groupsByCat[cat]}</div>`; });
    ...
    document.getElementById('settings-select-all-btn')?.addEventListener('click', () => {
      ...
      document.querySelectorAll('#settings-subjects .subject-check').forEach(c => { const g = c.closest('.setup-group'); c.checked = g.querySelectorAll('.paper-check').length === g.querySelectorAll('.paper-check:checked').length; });
    });
    ...
    document.getElementById('settings-subjects').innerHTML = '<p style="color:var(--text2);font-size:14px">No subjects in this exam set. Add papers in the custom exam editor.</p>';
  }

  // Colors
  const colorContainer = document.getElementById('settings-colors');
  const allSubj = [...new Set(papers.map(e => e.subject))].sort();
  colorContainer.innerHTML = allSubj.map(s => `<div class="color-row"><label>${s}</label><input type="color" class="subject-color-input" data-subject="${s}" value="${getSubjectColor(s)}"></div>`).join('');

  // Goals
  const goalContainer = document.getElementById('settings-goals');
  goalContainer.innerHTML = allSubj.map(s => `<div class="goal-row" style="flex-wrap:wrap"><span style="min-width:140px;font-size:14px;font-weight:500">${s}</span><span style="font-size:12px;color:var(--text2)">${getGoals(s).length ? getGoals(s).filter(g => !g.completed).length + ' pending' : 'No goals'}</span></div>`).join('');
}
...
function showExamDetail(examId) {
  const exam = getPapers().find(e => e.id === examId);
  if (!exam) return;
  const days = daysUntilExam(exam), status = getExamStatus(exam), color = getSubjectColor(exam.subject);
...
function exportData() {
  const set = state.examSets[state.activeSetId];
  if (!set) return;
  const data = JSON.stringify({ selectedIds: set.selectedIds, progress: set.progress, notes: set.notes, goals: set.goals, subjectColors: set.subjectColors, darkMode: state.darkMode, notifications: state.notifications, exportedAt: new Date().toISOString() }, null, 2);
  const blob = new Blob([data], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `exam-config-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url);
}
...
function refreshExamSwitcher() {
  ...
  sel.innerHTML = Object.entries(state.examSets).map(([id, set]) => `<option value="${id}"${id === state.activeSetId ? ' selected' : ''}>${set.name}</option>`).join('');
  sel.addEventListener('change', () => { switchExamSet(sel.value); });
  const emailEl = document.getElementById('user-email');
  const logoutBtn = document.getElementById('btn-logout');
  if (state.email) { emailEl.textContent = state.email; emailEl.classList.remove('hidden'); logoutBtn.classList.remove('hidden'); }
  else { emailEl.classList.add('hidden'); logoutBtn.classList.add('hidden'); }
}
...
function refreshAll() {
  ...
  renderStatsBar();
  renderGoals();
  renderList(state.examSets[state.activeSetId]?.selectedIds || []);
  renderCalendar();
...
async function runOCR(file) {
  ...
  const entriesDiv = document.getElementById('ocr-entries');
  ...
  try {
    ...
    if (extracted.length === 0) {
      statusEl.textContent = 'Could not find any exam entries. Try a clearer image.';
      uploadArea.classList.remove('hidden');
      return;
    }

    statusEl.textContent = `Found ${extracted.length} entries. Review and edit below.`;
    entriesDiv.innerHTML = extracted.map((entry, i) => `<div class="ocr-entry">
      <input class="ocr-date-input" type="date" value="${entry.date}" data-idx="${i}">
      <input class="ocr-subject-input" type="text" value="${entry.subject}" placeholder="Subject" data-idx="${i}">
      <input class="ocr-paper-input" type="text" value="${entry.paper || ''}" placeholder="Paper" data-idx="${i}">
      <input class="ocr-time-input" type="time" value="${entry.time || ''}" data-idx="${i}">
    </div>`).join('');
...
function parseExamLines(lines) {
  const results = [];
  // Match various date patterns: "14 Oct", "14 October", "14 October 2026", "14/10/2026", "2026-10-14"
  const datePatterns = [
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const pattern of datePatterns) {
      ...
      if (m) {
        if (m[2] && monthMap[m[2].toLowerCase().slice(0,3)] !== undefined) {
          ...
          const month = String(monthMap[m[2].toLowerCase().slice(0,3)] + 1).padStart(2, '0');
          const year = m[3] || '2026';
          dateStr = `${year}-${month}-${day}`;
        } else if (m[1] && m[2] && m[3] && m[1].length === 4) {
    ...
    // Clean subject name
    let subject = line.replace(/^\d{1,2}\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s*(\d{4})?\s*/gi, '');
    subject = subject.replace(/^\d{4}[-/]\d{1,2}[-/]\d{1,2}\s*/g, '');
    subject = subject.replace(/^\d{1,2}[-/]\d{1,2}[-/]\d{4}\s*/g, '');
    // Remove day names
    dayNames.forEach(d => { subject = subject.replace(new RegExp(d, 'gi'), ''); });
    // Remove time patterns
    subject = subject.replace(/\d{1,2}:\d{2}(?:\s*(?:am|pm))?/gi, '');
    // Clean up
    ...
    if (dateStr && subject && subject.length > 1) {
      // Avoid duplicates
      if (!results.some(r => r.date === dateStr && r.subject === subject)) {
        results.push({ date: dateStr, subject, paper: '', time });
      }
    }
  }
  return results;
...
(async () => {
  ...
  document.querySelectorAll('.login-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      confirmEl.classList.toggle('hidden', tab.dataset.tab === 'signin');
      document.getElementById('auth-email').value = '';
      document.getElementById('auth-password').value = '';
      document.getElementById('auth-password-confirm').value = '';
      document.getElementById('login-error').classList.add('hidden');
      document.getElementById('btn-auth').textContent = tab.dataset.tab === 'signin' ? 'Sign In' : 'Sign Up';
    });
  });
  ...
  ['auth-email', 'auth-password', 'auth-password-confirm'].forEach(id => {
    document.getElementById(id).addEventListener('keydown', (e) => { if (e.key === 'Enter') document.getElementById('btn-auth').click(); });
  });

  // Logout
  document.getElementById('btn-logout').addEventListener('click', handleLogout);

  // Refresh if not logged in (skip login already ran)
  if (!state.user) {
    if (!state.activeSetId) {
      state.examSets['builtin-o-level'] = { name: 'GCE O-Level 2026', type: 'builtin', builtinKey: 'o-level', selectedIds: [], progress: {}, notes: {}, goals: {}, subjectColors: {} };
      state.activeSetId = 'builtin-o-level';
  ...
  document.querySelectorAll('.view-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      else if (view === 'notes') { document.getElementById('notes-view').classList.add('active'); renderNotes(); }
  ...
  document.getElementById('btn-empty-settings').addEventListener('click', () => {
    document.querySelector('.view-tab[data-view="all"]')?.click();
  ...
  document.getElementById('settings-darkmode').addEventListener('change', () => {
  ...
  document.getElementById('btn-notifications').addEventListener('click', () => {
    ...
    if (Notification.permission === 'denied') { showToast('Notifications were blocked. Allow them in your browser settings.', 4000); return; }
    ...
    Notification.requestPermission().then(perm => {
      if (perm === 'granted') { state.notificationsEnabled = true; state.notifications = true; saveState(); showToast('Notifications enabled!'); }
    });
  });
  document.getElementById('settings-notifications').addEventListener('change', () => {
    state.notifications = document.getElementById('settings-notifications').checked;
    saveState();
  });
  ...
  document.getElementById('btn-goals-toggle').addEventListener('click', () => {
    const set = state.examSets[state.activeSetId];
    if (set) { set.goalsCollapsed = !set.goalsCollapsed; saveState(); renderGoals(); }
  });

  // Filter / sort / search
  document.getElementById('filter-subject').addEventListener('change', () => refreshAll());
  document.getElementById('sort-by').addEventListener('change', () => renderList(state.examSets[state.activeSetId]?.selectedIds || []));
  document.getElementById('search-exams').addEventListener('input', () => renderList(state.examSets[state.activeSetId]?.selectedIds || []));
  ...
  document.getElementById('btn-do-import').addEventListener('click', () => {
    ...
    else showToast('Select a JSON file first.');
  });
  ...
  document.getElementById('btn-feedback-open').addEventListener('click', () => {
    window.open(state.googleFormUrl || GOOGLE_FORM_URL, '_blank');
  });
  ...
  document.getElementById('btn-add-exam-set').addEventListener('click', () => {
    ...
    container.innerHTML = `<div class="exam-set-row"><input type="text" class="es-subject" placeholder="Subject" style="flex:2"><input type="text" class="es-paper" placeholder="Paper" style="flex:1"><input type="date" class="es-date" style="flex:1"><input type="time" class="es-time" placeholder="Time" style="flex:0.7"><button class="btn btn-small btn-danger es-remove">&times;</button></div>`;
  ...
  document.getElementById('btn-save-exam-set').addEventListener('click', () => {
    ...
    if (!name) { showToast('Please enter a name for the exam set.'); return; }
    const rows = document.querySelectorAll('#exam-set-entries .exam-set-row');
    const papers = [];
    rows.forEach(row => {
      const subject = row.querySelector('.es-subject').value.trim();
      const paper = row.querySelector('.es-paper').value.trim();
      const date = row.querySelector('.es-date').value;
      const time = row.querySelector('.es-time').value;
      if (subject && date) papers.push({ subject, paper, paperName: paper, date, startTime: time || null });
    });
    ...
    showToast('Custom exam set created!');
  });

  // Handle remove buttons for initial row
  ...
  document.getElementById('btn-ocr-start').addEventListener('click', async () => {
    const file = document.getElementById('ocr-file-input').files[0];
    if (!file) { showToast('Please select an image or PDF file first.'); return; }
    await runOCR(file);
  });
  document.getElementById('btn-ocr-save').addEventListener('click', () => {
    ...
    entries.forEach(row => {
      ...
      const subject = row.querySelector('.ocr-subject-input').value.trim();
      const paper = row.querySelector('.ocr-paper-input').value.trim();
      const time = row.querySelector('.ocr-time-input').value;
      if (date && subject) papers.push({ subject, paper, paperName: paper, date, startTime: time || null });
    });
    if (!papers.length) { showToast('No valid entries to save.'); return; }
    const defaultName = 'Imported ' + new Date().toLocaleDateString('en-SG');
    createCustomExamSet(defaultName, papers);
    document.getElementById('ocr-modal').classList.add('hidden');
    showToast('Exam set created from OCR!');
  });
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === '?' && !e.ctrlKey && !e.metaKey) {
      showToast('Shortcuts: ? Help | Esc Close modal | D Dark mode', 4000);
    }
    if ((e.key === 'd' || e.key === 'D') && !e.ctrlKey && !e.metaKey && !e.target.closest('input,textarea,select')) {
      state.darkMode = !state.darkMode; saveState(); refreshAll();
    }
  });

  // Print styles: hide overlays
  window.matchMedia('print').addEventListener('change', (mql) => {
    if (mql.matches) { document.querySelectorAll('.overlay').forEach(o => o.classList.add('hidden')); }
  });
  ...
  // Sync to Supabase periodically if logged in
  setInterval(() => { if (currentUser && supabase) syncToSupabase(); }, 300000);
